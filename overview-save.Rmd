...
Please ask questions and make comments as we go through the material. We'll also have an online shared document (Etherpad) at [https://v.etherpad.org/p/nimble-ncar-2016](https://v.etherpad.org/p/nimble-sbss-2016) where you can share code snippets and error messages, but I won't be monitoring that while I'm talking, so if there's something to look at there, let me know.
...
Here are the modules we'll work through for this workshop, though we probably won't get to all of them.
 ...
 - Introduction to NIMBLE 
    1. [The BUGS language](bugs_language_slides.html) 
       - [Exercise: writing BUGS code](exercises_slides.html)
    ...
    5. [Customizing an MCMC](customizing_mcmc_slides.html)
       - [Modifying MCMC samplers](exercises_slides.html)
...
Exercises can be found [here](exercises_slides.html); we'll work on some of these during the workshop.
    
Other topics:

1. [Constants in BUGS code](constants_slides.html)
2. [An extended MCMC example](litters_example_slides.html)
3. [Vectorizing BUGS code](vectorizing_slides.html)
4. [User-defined functions in BUGS code](user_function_slides.html)
5. [Compiling R code](compile_R_slides.html)
