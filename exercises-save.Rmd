1) Here's a basic generalized linear mixed model (a regression style model with random effects and non-normal likelihood). Your task is to write down the BUGS code corresponding to this verbal description of the model.

Consider an experiment with 16 litters of rats in one group and 16 litters in another group. Let `i` index the groups and `j` index the litters. Now suppose that there are `n[i,j]` pups born in each litter and there is a survival probability specific to each litter. Use this information to define the likelihood component of the model.
...
2) Define a model in NIMBLE using the BUGS code from (1). Then run a basic MCMC for the model. How does the MCMC look?
...
3) Now try other samplers for some of the parameters in the model.

(a) Try slice samplers on each of the hyperparameters, `a[1],a[2],b[1],b[2]`. 
(b) Try block samplers on `{a[1],b[1]}` and `{a[2],b[2]}`.
