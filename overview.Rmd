...
The Github site ([https://github.com/paciorek/nimble-ncar-2016](https://github.com/paciorek/nimble-ncar-2016)) is the master repository for materials for the modules; you can also get the individual files from Github. 

To get the materials look ahead to slide 6 for instructions. Alternatively, simply go to [https://github.com/paciorek/nimble-ncar-2016](https://github.com/paciorek/nimble-ncar-2016) and click on the 'Clone or Download' button in the right side of the window and then on 'Download ZIP', or simply click [here](https://github.com/paciorek/nimble-ncar-2016/archive/master.zip).



## Asking questions and making comments

Please ask questions and make comments as we go through the material. We'll also have an online shared document (Etherpad) at [https://v.etherpad.org/p/nimble-ncar-2016](https://v.etherpad.org/p/nimble-ncar-2016) where you can share code snippets and error messages, but I won't be monitoring that while I'm talking, so if there's something to look at there, let me know.

# How we'll operate

The workshop will be organized in modules, each of which will be a combination of lecture/demo presentation, in some cases followed by a short breakout session.
...
Simply go to [https://github.com/paciorek/nimble-ncar-2016](https://github.com/paciorek/nimble-ncar-2016) and click on the 'Download ZIP' button in the right side of the window or simply click [here](https://github.com/paciorek/nimble-ncar-2016/archive/master.zip).

## Within RStudio

Within RStudio go to File->New Project->Version Control->Git and enter:

- "Repository URL": https://github.com/paciorek/nimble-ncar-2016
- "Project Directory Name": nimble-ncar-2016 (or something else of your choosing)
...
- `git clone https://github.com/paciorek/nimble-ncar-2016`

Then to update from the repository to get any changes we've made:

- `cd /directory/where/you/put/the/repository/nimble-ncar-2016`
- `git pull`
...
describes how to install *Rtools* on Windows (be sure to check the box that will modify your PATH) and how to install the
...
* NIMBLE can take a long time to build models that have tens of thousands or more nodes (though once built, the algorithm run times can be quite good). There should be big improvements in this regard in a release of NIMBLE coming in the next month or so.
...
Here are the modules we'll work through for this workshop, though we probably won't get to all of them.
 ...
 - Introduction to NIMBLE 
    1. [The BUGS language](bugs_language_slides.html) 
        - [Exercise: writing BUGS code](exercises_slides.html#(1))
    2. [Building a model in NIMBLE](build_model_slides.html)
    3. [MCMC concepts](mcmc_concepts_slides.html)
        - [Exercise: building a model](exercises_slides.html#(2))
    4. [Running an MCMC](run_mcmc_slides.html)
    5. [Customizing an MCMC](customizing_mcmc_slides.html)
        - [Exercise: Modifying MCMC samplers](exercises_slides.html#(3))
 - More details of specifying BUGS models
    1. [Alternative parameterizations](alt_param_slides.html)
    2. [Indexing](indexing_slides.html)
    3. [User-defined distributions](user_dist_slides.html)
 - Programming in NIMBLE
    ...
    4. [Writing your own MCMC sampler](user_sampler_slides.html)

Other topics:

1. [Specifying data](data_nodes_slides.html)
2. [Constants in BUGS code](constants_slides.html)
3. [An extended MCMC example](litters_example_slides.html)
4. [Vectorizing BUGS code](vectorizing_slides.html)
5. [User-defined functions in BUGS code](user_function_slides.html)
6. [Compiling R code](compile_R_slides.html)
